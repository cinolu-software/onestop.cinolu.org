        ...
        switchMap(({ payload, onSuccess }) =>
          _http.patch<{ data: IRole }>(`roles/${payload.id}`, payload).pipe(
            map(({ data }) => {
              ...
              patchState(store, { isLoading: false, roles: [updated, count], allRoles: allUpdated });
        ...
        switchMap((id) =>
          _http.delete<void>(`roles/${id}`).pipe(
            map(() => {
              ...
              patchState(store, { isLoading: false, roles: [filtered, Math.max(0, count - 1)], allRoles: allFiltered });
