...
interface IArticlesStore {
  ...
  gallery: IImage[];
  // Unpaginated tags for selects/filters
  ...
  withMethods(({ _http, _router, _toast, ...store }) => ({
    ...
    loadPublishedArticles: rxMethod<FilterArticlesDto>(
      pipe(
        ...
        switchMap((queryParams) => {
          const params = buildQueryParams(queryParams);
          return _http.get<{ data: [IArticle[], number] }>('articles/find-published', { params }).pipe(
            map(({ data }) => {
              patchState(store, { isLoading: false, articles: data });
            }),
            catchError(() => {
    ...
    loadTags: rxMethod<void>(
      pipe(
        tap(() => patchState(store, { isLoadingTags: true } as Partial<IArticlesStore>)),
        switchMap(() =>
          _http.get<{ data: ITag[] }>('tags/filtered').pipe(
            map(({ data }) => {
              patchState(store, { isLoadingTags: false, tags: data } as Partial<IArticlesStore>);
            }),
            catchError(() => {
              patchState(store, { isLoadingTags: false, tags: [] } as Partial<IArticlesStore>);
              return of(null);
            })
          )
        )
